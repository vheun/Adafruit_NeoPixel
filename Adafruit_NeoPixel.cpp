...
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, uint8_t p, uint8_t t) {
  ...
  if((pixels = (uint8_t *)malloc(numBytes))) {
    ...
    type    = t;
    pin     = p;
#ifndef __SAM3X8E__ // Arduino Due dose dose not compile with the following commands
...
#ifdef __arm__
// Set the pins of the Arduino Due super fast. 
...
// Arduino Due cli() and sei() are not supported in order to touch minimum
// original code anything is just encapsulated.
